  ...
  useEffect(() => {
    ...
    const callInterval = setInterval(() => {
      if (!activeCall && Math.random() > 0.1) {
        const callerNames = ['Ahmed El Sayed', 'Jane Smith'];
        const roles = ['Scad Officer'];
        const purposes = ['Career Guidance', 'Report Clarification'];
    ...
    <div className="dashboard-wrapper" role="main">
      <div className={`sidebar ${sidebarOpen ? 'open' : 'closed'}`}>
        <div className="sidebar-header">
          <Link to="/pro-student-dashboard" className="dashboard-redirect-btn" aria-label="Go back to SCAD Dashboard">
            <FiHome className="dashboard-icon" />  Dashboard
          </Link>
        </div>
